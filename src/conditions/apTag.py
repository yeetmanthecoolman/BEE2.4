...
def res_make_tag_coop_spawn(vmf: VMF, inst: Entity, res: Property):
    ...
    It produces either an instance or the normal spawn entity.
    This is required since ATLAS may need to have the paint gun logic.
...
def calc_fizzler_orient(fizzler: Fizzler) -> Tuple[str, float, float, float]:
    """Figure out where the signs should appear for this fizzler."""
...
def res_make_tag_fizzler(vmf: VMF, res: Property):
    ...
    import vbsp
    if 'ioconf' in res:
        fizz_io_type = ItemType.parse('<TAG_FIZZER>', res.find_key('ioconf'))
    else:
        fizz_io_type = None
